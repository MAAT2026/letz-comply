    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="text-center">
          <img src="/logo.svg" alt="LetzComply logo" className="w-12 h-12 mx-auto rounded-lg" />
          <h1 className="mt-3 text-xl font-bold text-slate-900">Welcome to LetzComply</h1>
        ...
        <div className="text-center text-sm text-slate-500 space-y-1">
          {mode === 'login' && (
            <>
              <button type="button" onClick={() => setMode('forgot')} className="hover:text-slate-800 transition-colors">Forgot password?</button>
              <p>Need an account? <button type="button" onClick={() => setMode('signup')} className="text-cyan-600 font-medium hover:text-cyan-800">Sign up</button></p>
            </>
          )}
          {mode === 'signup' && (
            <p>Already have an account? <button type="button" onClick={() => setMode('login')} className="text-cyan-600 font-medium hover:text-cyan-800">Sign in</button></p>
          )}
          {mode === 'forgot' && (
            <p><button type="button" onClick={() => setMode('login')} className="text-cyan-600 font-medium hover:text-cyan-800">Back to sign in</button></p>
          )}
        </div>
      </div>
    </div>
